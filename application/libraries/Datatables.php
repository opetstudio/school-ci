    ...
    */
    private function get_filtering()
    {
      ...
      if($sSearch != ''){
        ...
        if($mColArray[1]['search']['value'] |= ''){
          for($i = 1; $i < count($mColArray); $i++){
            //print_r($mColArray[$i]['data']); die();
            if($mColArray[$i]['search']['value'] |= '')
                $cari = $mColArray[$i]['search']['value'];
                //print_r($mColArray[$i]); die();
                $sWhere .= $this->select[$mColArray[$i]['data']] . " LIKE '%" . $cari . "%' OR ";
          }
        }
    ...
    private function get_total_results($filtering = FALSE)
    {
      ...
      foreach($this->or_like as $val)
        ...
        if (strlen($this->distinct) > 0) {
            ...
            $this->ci->db->select("count(*) as jml");
            return findOne($this->ci->db->get_compiled_select($this->table))->jml;
        }
    }
    /**
...
/* Generated by Harviacode Codeigniter CRUD Generator 2017-12-14 14:32:59 */
/* http://harviacode.com */