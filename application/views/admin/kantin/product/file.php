<form id="productForm" method="post" action="<?=base_url('api/kantin/product/'.$action); ?>" class="form-horizontal">
...
$(document).ready(function() {
    ...
    var foto_upload = new Dropzone(".dropzone", {
        url: ($('#productForm').attr('action') + '/' + $('#productForm input[name="id"]').val()),
        // params: ,
        method: 'POST',
        headers: {
        ...
        init: function() {
            ...
            attach.forEach(function(a, b) {
                var src = '<?= base_url(PATH_PUBLIC_ATTACH . 'product/') ?>' + a;
                console.log(src)
                if (Main.imageExists(src)) {
                    ...
                    setTimeout(() => {
                    $('.dropzone .dz-details').attr('path','<?= base_url(PATH_PUBLIC_ATTACH . 'product/') ?>')
                }, 300);
                }
            })
           ...
           this.on("removedfile", function(a) {
            ...
            $.ajax({
                ...
                url: '<?= base_url('api/kantin/product/deletefile/') ?>' + a._removeLink.id,
                // cache: false,
                //                        dataType: 'json',
