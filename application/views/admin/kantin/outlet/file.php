<form id="outletForm" method="post" action="<?=base_url('api/kantin/kantin/'.$action); ?>" class="form-horizontal">
...
$(document).ready(function() {
    ...
    var foto_upload = new Dropzone(".dropzone", {
        url: ($('#outletForm').attr('action') + '/' + $('#outletForm input[name="id"]').val()),
        // params: ,
        method: 'POST',
        headers: {
        ...
        init: function() {
            ...
            attach.forEach(function(a, b) {
                var src = '<?= base_url(PATH_PUBLIC_ATTACH . 'outlet/') ?>' + a;
                console.log(src)
                if (Main.imageExists(src)) {
                    ...
                    setTimeout(() => {
                    $('.dropzone .dz-details').attr('path','<?= base_url(PATH_PUBLIC_ATTACH . 'outlet/') ?>')
                }, 300);
                }
            })
           ...
           this.on("removedfile", function(a) {
            ...
            $.ajax({
                ...
                url: '<?= base_url('api/kantin/kantin/deletefile/') ?>' + a._removeLink.id,
                // cache: false,
                //                        dataType: 'json',
