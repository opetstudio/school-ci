<form id="bagivideoForm" method="post" action="<?=base_url('api/forum/berbagi/'.$action); ?>" class="form-horizontal">
...
$(document).ready(function() {
    ...
    var foto_upload = new Dropzone(".dropzone", {
        url: ($('#bagivideoForm').attr('action') + '/' + $('#bagivideoForm input[name="id"]').val()),
        // params: ,
        method: 'POST',
        headers: {
        ...
        init: function() {
            ...
            attach.forEach(function(a, b) {
                var src = '<?= base_url(PATH_PUBLIC_ATTACH . 'berbagi/') ?>' + a;
                console.log(src)
                if (Main.imageExists(src)) {
                    ...
                    setTimeout(() => {
                    $('.dropzone .dz-details').attr('path','<?= base_url(PATH_PUBLIC_ATTACH . 'berbagi/') ?>')
                }, 300);
                }
            })
            ...
            $.ajax({
                ...
                url: '<?= base_url('api/forum/berbagi/deletevideo/') ?>' + a._removeLink.id,
                // cache: false,
                //                        dataType: 'json',
