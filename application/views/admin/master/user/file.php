...
$(document).ready(function() {
    ...
    var foto_upload = new Dropzone(".dropzone", {
        url: ($('#userForm').attr('action') + '/' + $('#userForm input[name="id"]').val()),
        // params: ,
        method: 'POST',
        headers: {
        ...
        init: function() {
            ...
            attach.forEach(function(a, b) {
                var src = '<?= base_url(PATH_PUBLIC_ATTACH . 'user/') ?>' + a;
                console.log(src)
                if (Main.imageExists(src)) {
                    ...
                    setTimeout(() => {
                    $('.dropzone .dz-details').attr('path','<?= base_url(PATH_PUBLIC_ATTACH . 'user/') ?>')
                }, 300);
                }
            })
           ...
           this.on("removedfile", function(a) {
            ...
            $.ajax({
                ...
                url: '<?= base_url('api/master/user/deletefile/') ?>' + a._removeLink.id,
                // cache: false,
                //                        dataType: 'json',
