<form id="notifForm" method="post" action="<?=base_url('api/master/notif/'.$action); ?>" class="form-horizontal">
...
$(document).ready(function() {
    ...
    var foto_upload = new Dropzone(".dropzone", {
        url: ($('#notifForm').attr('action') + '/' + $('#notifForm input[name="id"]').val()),
        // params: ,
        method: 'POST',
        headers: {
        ...
        init: function() {
            ...
            attach.forEach(function(a, b) {
                var src = '<?= base_url(PATH_PUBLIC_ATTACH . 'notif/') ?>' + a;
                console.log(src)
                if (Main.imageExists(src)) {
                    ...
                    setTimeout(() => {
                    $('.dropzone .dz-details').attr('path','<?= base_url(PATH_PUBLIC_ATTACH . 'notif/') ?>')
                }, 300);
                }
            })
           ...
           this.on("removedfile", function(a) {
            ...
            $.ajax({
                ...
                url: '<?= base_url('api/master/notif/deletefile/') ?>' + a._removeLink.id,
                // cache: false,
                //                        dataType: 'json',
