...
$(document).ready(function() {
    ...
    var foto_upload = new Dropzone(".dropzone", {
        url: ($('#informasiForm').attr('action') + '/' + $('#informasiForm input[name="id"]').val()),
        // params: ,
        method: 'POST',
        headers: {
        ...
        init: function() {
            ...
            attach.forEach(function(a, b) {
                var src = '<?= base_url(PATH_PUBLIC_ATTACH . 'informasi/') ?>' + a;
                console.log(src)
                if (Main.imageExists(src)) {
                    ...
                    setTimeout(() => {
                    $('.dropzone .dz-details').attr('path','<?= base_url(PATH_PUBLIC_ATTACH . 'informasi/') ?>')
                }, 300);
                }
            })
           ...
           this.on("removedfile", function(a) {
            ...
            $.ajax({
                ...
                url: '<?= base_url('api/master/informasi/deletefile/') ?>' + a._removeLink.id,
                // cache: false,
                //                        dataType: 'json',
