...
$(document).ready(function() {
    ...
    $('#mytable').DataTable({
        ...
        buttons: [{
            ...
            init: function(a, b, c) {
                b.attr('href', __base_url + 'admin/akademik/pindah_sekolah/create');
                b.attr('title', 'CREATE');
            },
        }, ],
        "processing": true,
        "serverSide": true,
        "ajax": {
            "url": __base_url + "api/akademik/pindah_sekolah/json",
            "type": "POST",
            "headers": {
        ...
        "columns": [{
            ...
            },
            {
                ...
                "render": function(row, data, iDisplayIndex) {
                    var result = "";
                    result += "<button href=\"" + __base_url + "admin/akademik/pindah_sekolah/read/" +
                        iDisplayIndex.id +
                        "\" class=\"btn btn-info btn-read btn-xs\" title=\"Read\"><i class=\"fa fa-eye\"></i> Read</button>";

                    result += "<button href=\"" + __base_url + "admin/akademik/pindah_sekolah/update/" +
                        iDisplayIndex.id +
                        "\" class=\"btn btn-warning btn-update btn-xs\" title=\"Update\"><i class=\"fa fa-pencil\"></i> Update</button>";

                    result += "<button href=\"" + __base_url + "admin/akademik/pindah_sekolah/delete/" +
                        iDisplayIndex.id +
    ...
    function checkButton() {
        setTimeout(() => {
            ...
            if (id_skl && id_skl && id_semester && id_jurusan && id_kelas) {
    ...
    $(document).on('click', form + ' .btn-save-upload', function(e) {
        ...
        $("#dvExcel tbody tr").each(function(index) {
            ...
            if (thiss.find('input[name="ambil[]"]').is(':checked')) {
                ...
                delete value.id;
                $.ajax({
                    url: __base_url + "api/akademik/pindah_sekolah/create",
                    data: {
                        data: JSON.stringify([value])
                    },
                    method: $(form).attr('method'),
