...
$(document).ready(function() {
    ...
    var foto_upload = new Dropzone(".dropzone", {
        url: ($('#rppForm').attr('action') + '/' + $('#rppForm input[name="id"]').val()),
        // params: ,
        method: 'POST',
        headers: {
        ...
        maxFilesize: 30,
        // acceptedFiles: "image/*",
        acceptedFiles: ".pdf,.xls,.xlsx,.doc,.docx",
        paramName: "attach",
        ...
        init: function() {
            ...
            attach.forEach(function(a, b) {
                var src = '<?= base_url(PATH_PUBLIC_ATTACH . 'rpp/') ?>' + a;
                ...
                setTimeout(() => {
                    $('.dropzone .dz-details').attr('path','<?= base_url(PATH_PUBLIC_ATTACH . 'rpp/') ?>')
                }, 300);
            })
            ...
            this.on("removedfile", function(a) {
                ...
                $.ajax({
                    ...
                    url: '<?= base_url('api/akademik/rpp/deletefile/') ?>' + a._removeLink.id,
                    // cache: false,
                    //                        dataType: 'json',
