...
$(document).ready(function() {
    ...
    var foto_upload = new Dropzone(".dropzone", {
        url: ($('#prosemForm').attr('action') + '/' + $('#prosemForm input[name="id"]').val()),
        // params: ,
        method: 'POST',
        headers: {
        ...
        init: function() {
            ...
            attach.forEach(function(a, b) {
                var src = '<?= base_url(PATH_PUBLIC_ATTACH . 'prosem/') ?>' + a;
                ...
                setTimeout(() => {
                    $('.dropzone .dz-details').attr('path','<?= base_url(PATH_PUBLIC_ATTACH . 'prosem/') ?>')
                }, 300);
            })
            ...
            this.on("removedfile", function(a) {
                ...
                $.ajax({
                    ...
                    url: '<?= base_url('api/akademik/prosem/deletefile/') ?>' + a._removeLink.id,
                    // cache: false,
                    //                        dataType: 'json',
