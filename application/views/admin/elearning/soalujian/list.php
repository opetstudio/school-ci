...
$(document).ready(function() {
    ...
    $('#mytable').DataTable({
        ...
        buttons: [{
            ...
            init: function(a, b, c) {
                b.attr('href', __base_url + 'admin/elearning/soalujian/create');
                b.attr('title', 'CREATE');
            },
        }, ],
        "processing": true,
        "serverSide": true,
        "ajax": {
            "url": __base_url + "api/elearning/soalujian/json",
            "type": "POST",
            "headers": {
        ...
        "columns": [{
            ...
            },
            {
                ...
                "render": function(row, data, iDisplayIndex) {
                    var result = "";
                    result += "<button href=\"" + __base_url + "admin/elearning/soalujian/read/" +
                        iDisplayIndex.id +
                        "\" class=\"btn btn-info btn-read btn-xs\" title=\"Read\"><i class=\"fa fa-eye\"></i> Read</button>";
                    result += "<a href=\"" + __base_url + "admin/elearning/soalujian/pilihan/" +
                        iDisplayIndex.id +
                        "\" class=\"btn btn-success btn-xs\" title=\"Pilihan\"><i class=\"fa fa-pencil\"></i> Pilihan</a>";
                    result += "<a href=\"" + __base_url + "admin/elearning/soalujian/essay/" +
                        iDisplayIndex.id +
                        "\" class=\"btn btn-success btn-xs\" title=\"Essay\"><i class=\"fa fa-pencil\"></i> Essay</a>";
                    result += "<button href=\"" + __base_url + "admin/elearning/soalujian/update/" +
                        iDisplayIndex.id +
                        "\" class=\"btn btn-warning btn-update btn-xs\" title=\"Update\"><i class=\"fa fa-pencil\"></i> Update</button>";
                    result += "<button href=\"" + __base_url + "admin/elearning/soalujian/delete/" +
                        iDisplayIndex.id +
    ...
    $(document).on('click', form + ' .btn-save', function(e) {
        ...
        if (value.mulai) {
            value.mulai = moment(value.mulai, "DD/MM/YYYY").format("YYYY/MM/DD")
        }
        if (value.selesai) {
            value.selesai = moment(value.selesai, "DD/MM/YYYY").format("YYYY/MM/DD")
        }
        $.ajax({
