<form id="bukuForm" method="post" action="<?=base_url('api/elearning/materi/'.$action); ?>" class="form-horizontal">
    ...
    <label class="text-danger"><b>Note: File uploaded limited to pdf and image only, with maximum capacity is 30
            MB.</b></label>
    </div>
</form>
...
$(document).ready(function() {
    ...
    var foto_upload = new Dropzone(".dropzone", {
        url: ($('#bukuForm').attr('action') + '/' + $('#bukuForm input[name="id"]').val()),
        // params: ,
        method: 'POST',
        headers: {
        ...
        init: function() {
            ...
            attach.forEach(function(a, b) {
                var src = '<?= base_url(PATH_PUBLIC_ATTACH . 'materi/') ?>' + a;
                console.log(src)
                if (Main.imageExists(src)) {
                    ...
                    setTimeout(() => {
                    $('.dropzone .dz-details').attr('path','<?= base_url(PATH_PUBLIC_ATTACH . 'materi/') ?>')
                }, 300);
                }
            })
            ...
            $.ajax({
                ...
                url: '<?= base_url('api/elearning/materi/deletefile/') ?>' + a._removeLink.id,
                // cache: false,
                //                        dataType: 'json',
