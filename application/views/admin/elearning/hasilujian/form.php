                        ...
                        if (value.flag == "ganda") {
                            ...
                            if (value.jawaban) {
                                JSON.parse(value.pilihan).forEach(function(value, i) {
                                    jawaban.push(
                                        '<input type="checkbox" name="ganda" style="pointer-events: none;" value="' + pil[i] + '"> ' + pil[i] + ". " + value);
                                });
                            }
                            ...
                            $('.tableLoadEssay tbody').append(`
                                <tr class="soal-` + value.id + `" data-id="` + value.id + `" data-nilai="`+value.nilai+`">
                                    <td>` + noEssay + `</td>
                                    <td>` + (value.file ? '<img src="' + __base_url +'assets/public/attach/soalujian/' + value.file + '" style="width:200px;">' : "") + `</td>
                                    <td>` + value.nilai + `</td>
                                    <td>` + value.pertanyaan + `</td>
                                    <td><textarea name="essay" class="form-control" rows="7" style="pointer-events: none;"></textarea></td>
                                    <td><input type="number" name="hasil" class="form-control" style="pointer-events: none;"></td>
                                </tr>
                            `)
                        }
                    });
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $.each(data.data, function(i, value) {
                                ...
                                jmNilai += parseInt(soal.attr('data-nilai'));
                                if (value.flag == "ganda") {
                                    ...
                                    if(value.ganda==soal.attr('data-jawaban')){
                                        ...
                                        jmHasil += parseInt(soal.attr('data-nilai'));
                                    }
                                } else {
                                    soal.find('textarea[name="essay"]').val(value.essay);
                                    soal.find('input[name="hasil"]').val(value.hasil);
                                    jmHasil += parseInt(value.hasil);
                                }
                            })

                            $('.information tr:nth(0) td:nth(3)').text(jmNilai)
                            $('.information tr:nth(1) td:nth(3)').text(jmHasil)
                        }
                    })
                }
            }, 500);
