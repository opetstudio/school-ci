<form id="bukuForm" method="post" action="<?=base_url('api/elibrary/buku/'.$action); ?>" class="form-horizontal">
    ...
    var foto_upload = new Dropzone(".dropzone", {
        ...
        init: function() {
            ...
            attach.forEach(function(a, b) {
                var src = '<?= base_url(PATH_PUBLIC_ATTACH . 'buku/') ?>' + a;
                console.log(src)
                if (Main.imageExists(src)) {
                    ...
                    setTimeout(() => {
                    $('.dropzone .dz-details').attr('path','<?= base_url(PATH_PUBLIC_ATTACH . 'buku/') ?>')
                }, 300);
                }
            })
            ...
            $.ajax({
                ...
                url: '<?= base_url('api/elibrary/buku/deletecover/') ?>' + a._removeLink.id,
                // cache: false,
                //                        dataType: 'json',
