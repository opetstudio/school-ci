<form id="videoForm" method="post" action="<?=base_url('api/anjungan/video/'.$action); ?>" class="form-horizontal">
...
$(document).ready(function() {
    ...
    var foto_upload = new Dropzone(".dropzone", {
        url: ($('#videoForm').attr('action') + '/' + $('#videoForm input[name="id"]').val()),
        // params: ,
        method: 'POST',
        headers: {
        ...
        init: function() {
            ...
            attach.forEach(function(a, b) {
                var src = '<?= base_url(PATH_PUBLIC_ATTACH . 'video/') ?>' + a;
                console.log(src)
                if (Main.imageExists(src)) {
                    ...
                    setTimeout(() => {
                    $('.dropzone .dz-details').attr('path','<?= base_url(PATH_PUBLIC_ATTACH . 'video/') ?>')
                }, 300);
                }
            })
            ...
            $.ajax({
                ...
                url: '<?= base_url('api/anjungan/video/deletecover/') ?>' + a._removeLink.id,
                // cache: false,
                //                        dataType: 'json',
