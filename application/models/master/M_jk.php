...
class M_jk extends CI_Model
{
    ...
    public $sql = "
                        select
                        jk.*,
                        if(jk.is_active=1,'YES','NO') as is_active_name,
                        muc.name as created_by_name,
                        muu.name as updated_by_name
                        from app_jk jk
                        left join app_user muc on muc.id = jk.created_by
                        left join app_user muu on muu.id = jk.updated_by
    ";
    public $where = [' jk.is_active != 9 '];
    public $add_params = [
